...
from utils.metrics_utils import autoreg_ksteps_metrics
...
def run_attack(attacker_model, reasoner_model, dataset, param_instance, output_dir=DEFAULT_DUMP_DIR):
    """Train the attacker model on the given dataset."""
    training_args = TrainingArguments(
        str(Path(output_dir, synexp_args_to_wandb_run_name(param_instance))),
        num_train_epochs = param_instance["num_epochs"],
        per_device_train_batch_size = param_instance["train_batch_size"],
        per_device_eval_batch_size = param_instance["eval_batch_size"],
    ...
    for param_dict in config:
        for param_tuple in itertools.product(*param_dict.values()):
            ...
            big_dataset = AutoregKStepsTokensAttackDataset(
                ...
                dataset_len=param_instance["train_len"] + param_instance["eval_len"],
            )

            train_dataset, eval_dataset = torch.utils.data.random_split(
                big_dataset, [param_instance["train_len"], param_instance["eval_len"]]
            )
