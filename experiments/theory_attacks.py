...
def get_file_name(param_instance):
    ...
    if param_instance["syn_exp_name"] == "autoreg_ksteps":
        return f"{param_instance['model_name']}" + \
...
@torch.no_grad()
def run_eval(model, dataset, param_instance, lambda_list=LAMBDA, output_dir=DEFAULT_DUMP_DIR):
    ...
    for batch in pbar:
        ...
        for lambda_val in lambda_list:
            ...
            u_atks = torch.cat((torch.zeros(tokens.size(0), 1, num_vars + 1), lambda_adjusted_s_tgts), dim=2)
            ...
            lambda_metrics[lambda_val]["states_hits"] += (torch.mean((preds == s_tgts).float(), axis=2) > 1 - 1e-5).sum()
            lambda_metrics[lambda_val]["target_running_hits"] += (preds[:,-1] == s_tgts[:,-1]).sum()
            lambda_metrics[lambda_val]["target_states_hits"] += (torch.mean((preds[:,-1] == s_tgts[:,-1]).float(), axis=1) > 1 - 1e-5).sum()
            lambda_metrics[lambda_val]["num_dones"] += tokens.size(0)

            target_elems_acc = lambda_metrics[lambda_val]["running_hits"].item() / (lambda_metrics[lambda_val]["num_dones"] * num_vars)
            target_states_acc = lambda_metrics[lambda_val]["target_states_hits"].item() / lambda_metrics[lambda_val]["num_dones"]
    ...
    acc_lambda_metrics = {
        lambda_val: {
            "ElemsAcc": lambda_metrics[lambda_val]["running_hits"].item() / (lambda_metrics[lambda_val]["num_dones"] * num_vars * param_instance["num_steps"]),
            "StatesAcc": lambda_metrics[lambda_val]["states_hits"].item() / (lambda_metrics[lambda_val]["num_dones"] * param_instance["num_steps"]),
            "TargetElemsAcc": lambda_metrics[lambda_val]["running_hits"].item() / (lambda_metrics[lambda_val]["num_dones"] * num_vars),
            "TargetStatesAcc": lambda_metrics[lambda_val]["target_states_hits"].item() / lambda_metrics[lambda_val]["num_dones"],
        } for lambda_val in lambda_list
    }
    ...
    for param_dict in config:
        for param_tuple in itertools.product(*param_dict.values()):
            ...
            if "syn_exp_name" in param_instance:
                assert param_instance["syn_exp_name"] == "autoreg_ksteps", "syn_exp_name must be autoreg_ksteps for this experiment."
            else:
                raise ValueError("syn_exp_name must be specified in config file.")
            
            dataset = AutoregKStepsTokensDataset(
                num_vars=param_instance["num_vars"],
                num_rules_range=(param_instance["min_num_rules"], param_instance["max_num_rules"]),
                ante_prob_range=(param_instance["min_ante_prob"], param_instance["max_ante_prob"]),
                conseq_prob_range=(param_instance["min_conseq_prob"], param_instance["max_conseq_prob"]),
                chain_len_range=(param_instance["min_chain_len"], param_instance["max_chain_len"]),
            ...
            run_eval(model, dataset, param_instance, lambda_list=args.lambda_list, output_dir=args.output_dir)
