    ...
    for param_dict in config:
        ...
        for param_tuple in itertools.product(*param_dict.values()):
            ...
            for sample_i, example in tqdm(enumerate(samples)):
                ...
                tokenized_input = tokenizer(input_to_model, return_tensors="pt").input_ids.to(args.device)
                tokenized_attack_input = tokenizer(attack_input_to_model, return_tensors="pt").input_ids.to(args.device)
                ...
                with torch.no_grad():
                    ...
                    for ntk in range(num_tokens_to_generate):
                        op_without_suffix = model(tokenized_input, output_attentions=True)
                        op_with_suffix = model(tokenized_attack_input, output_attentions=True)

                        # Get list of tokens for input and attack input
                        tokens_without_suffix = tokenizer.convert_ids_to_tokens(tokenized_input[0])
                        tokens_with_suffix = tokenizer.convert_ids_to_tokens(tokenized_attack_input[0])

                        
                        # Get the next token
                        next_token_without_suffix = op_without_suffix.logits[0, -1].argmax()
                        ...
                        print(f"Shape of attns_without_suffix: {attns_without_suffix[0].shape}")
                        print(f"Shape of attns_with_suffix: {attns_with_suffix[0].shape}")
                        ...
                        print(f"Shape of heatmap_without_suffix: {heatmap_without_suffix.shape}")
                        print(f"Shape of heatmap_with_suffix: {heatmap_with_suffix.shape}")
                        ...
                        os.makedirs(f"attention_heatmaps_supress_universal/{exp_id}/sample_{sample_i}_{j}", exist_ok=True)
                        plt.savefig(f"attention_heatmaps_supress_universal/{exp_id}/sample_{sample_i}_{j}/token_{ntk}.png")
                        ...
                        tokenized_input = torch.cat([tokenized_input, next_token_without_suffix.unsqueeze(0).unsqueeze(0)], dim=-1).to(args.device)
                        tokenized_attack_input = torch.cat([tokenized_attack_input, next_token_with_suffix.unsqueeze(0).unsqueeze(0)], dim=-1).to(args.device)
            ...
            for layer in range(12):
                ...
                for sample_i in range(len(samples)):
                    ...
                    axs[layer // 3, layer % 3].scatter(sample_i_data[:, 2], sample_i_data[:, 3], label=f"Sample {sample_i}")

                # Also plot the y = x line
                axs[layer // 3, layer % 3].plot([0, 1], [0, 1], color="red", linestyle="--")

            # Add a legend
            axs[0, 0].legend()

            plt.savefig(f"attention_heatmaps_supress_universal/attack_stats_{exp_id}.png")
        ...
        for layer in range(12):
            ...
            for head in range(12):
                ...
                for sample_i in range(len(samples)):
                    ...
                    axs[head // 3, head % 3].scatter(sample_i_data[:, 2], sample_i_data[:, 3], label=f"Sample {sample_i}")

                # Also plot the y = x line
                axs[head // 3, head % 3].plot([0, 1], [0, 1], color="red", linestyle="--")

            plt.savefig(f"attention_heatmaps_supress_universal/attack_stats_{exp_id}_layer_{layer}.png")
