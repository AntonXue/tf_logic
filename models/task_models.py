...
""" Specialized task model for small transformers """

@dataclass
class SmallTfSuccTaskOutput(ModelOutput):
    loss: Optional[torch.FloatTensor] = None
    logits: Optional[torch.FloatTensor] = None


class SmallTfSuccTaskModel(SeqClsModel):
    def __init__(self, num_vars: int):
        ...
        self.attn_activ = F.sigmoid
        # model output: y = activ(mask(X Wa X')) X Wb
    ...
    def forward(
        ...
        wts = self.attn_activ(torch.bmm(self.Wa(x), x.transpose(1,2)) + mask) # activ(mask(x @ Wa @ x))
        a = torch.bmm(wts, x)
        y = self.Wb(a)
        logits = y[:,-1]
        ...
        if labels is not None:
            ...
            labels = 2 * labels - 1
            loss = F.relu(1 -logits * labels).mean()  # max(0, 1 - y_pred * y_true)

        return SmallTfSuccTaskOutput(
            loss = loss,
            logits = logits
        )
