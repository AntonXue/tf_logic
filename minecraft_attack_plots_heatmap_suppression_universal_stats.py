...
def plot_attention_diffs(
    attns_without_suffix, attns_with_suffix, figsize=(3, 3), savepdf=None
):
    ...
    for i in range(len(attns_with_suffix)):
        ax.scatter(
            attns_with_suffix[i], attns_without_suffix[i], label=f"Sample {sample_i}"
        )
...
def plot_attention_flow(
    ...
    token_labels[-1] = ". . ."
    token_labels.extend(
        orig_token_labels[flow_matrix.shape[1] - 4 : flow_matrix.shape[1]]
    )
    token_labels[0] = ". . . "
    ...
    for i in range(flow_matrix.shape[0]):
        for j in range(len(token_labels)):
            ...
            # for i in range(1):
            text_color = (
                "w" if abs(flow_matrix[i, j]) > 0.75 else "k"
            )  # w=white, k=black

            text = ax.text(
                j + 0.5,
                i + 0.5,
                valfmt(flow_matrix[i, j]),
                ha="center",
                va="center",
                fontsize=10,
                color=text_color,
    ...
    if cbar_text:
        cb.ax.set_title(cbar_text, y=-0.16, fontsize=16)
    ...
    save_dict = {"flow_matrix": flow_matrix, "tokens": token_labels}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num_samples", type=int, default=100, help="Number of samples to visualize."
    )
    ...
    for param_dict in config:
        ...
        for param_tuple in itertools.product(*param_dict.values()):
            ...
            all_attns_without_suffix = {k: [] for k in range(12)}
            all_attns_with_suffix = {k: [] for k in range(12)}
            all_attns_diff = {k: [] for k in range(12)}
            ...
            for sample_i, example in tqdm(enumerate(samples[: args.num_samples])):
                ...
                tokenized_input = tokenizer(input_to_model, return_tensors="pt").to(
                    args.device
                )
                ...
                tokenized_facts_start = (
                    len(tokenizer(facts_start, return_tensors="pt").input_ids[0]) + 1
                )  # +1 to ignore the \n

                rules_start = input_to_model.split("If I have")[0]
                tokenized_rules_start = len(
                    tokenizer(rules_start, return_tensors="pt").input_ids[0]
                )

                rules_end = input_to_model.split("Here are some items")[0]
                tokenized_rules_end = len(
                    tokenizer(rules_end, return_tensors="pt").input_ids[0]
                )
                ...
                for rule in recipe_suppressed:
                    if rule[2] == param_instance["num_steps"] - 1:
                        ...
                        antecedents = [
                            a.replace("minecraft:", "").replace("_", " ")
                            for a in antecedents
                        ]
                        consequent_suppressed = consequent_suppressed.replace(
                            "minecraft:", ""
                        ).replace("_", " ")
                        ...
                        suppresed_rule_pos = [
                            i
                            for i, rule in enumerate(input_to_model.split("."))
                            if f"then I can create {consequent_suppressed}" in rule
                            and all([ante in rule for ante in antecedents])
                        ]
                        ...
                        suppressed_rule_start = (
                            ".".join(input_to_model.split(".")[: suppresed_rule_pos[0]])
                            + "."
                        )
                        print(f"Suppressed rule start: {suppressed_rule_start}")

                        tokenized_suppressed_rule_start = len(
                            tokenizer(
                                suppressed_rule_start, return_tensors="pt"
                            ).input_ids[0]
                        )

                        suppressed_rule_end = ".".join(
                            input_to_model.split(".")[: suppresed_rule_pos[0] + 1]
                        )
                        print(f"Suppressed rule end: {suppressed_rule_end}")

                        tokenized_suppressed_rule_end = len(
                            tokenizer(
                                suppressed_rule_end, return_tensors="pt"
                            ).input_ids[0]
                        )

                        suppressed_rule_token_pos.extend(
                            list(
                                range(
                                    tokenized_suppressed_rule_start,
                                    tokenized_suppressed_rule_end,
                                )
                            )
                        )

                sample_attns_without_suffix = {k: 0 for k in range(12)}
                sample_attns_with_suffix = {k: 0 for k in range(12)}
                sample_attns_diff = {k: -999 for k in range(12)}
                ...
                with torch.no_grad():
                    ...
                    op_without_suffix = model.generate(
                        **tokenized_input,
                    ...
                    for ntk in tqdm(range(num_tokens_to_generate)):
                        ...
                        tokens_without_suffix = [
                            tokenizer.decode(t)
                            for t in op_without_suffix.sequences[0][
                                : -(num_tokens_to_generate - ntk)
                            ]
                        ]
                        tokens_with_suffix = [
                            tokenizer.decode(t)
                            for t in op_with_suffix.sequences[0][
                                : -(num_tokens_to_generate - ntk)
                            ]
                        ]
                        print(f"Shape of tokens with suffix: {len(tokens_with_suffix)}")
                        print(
                            f"Shape of tokens without suffix: {len(tokens_without_suffix)}"
                        )

                        tks_without.append(
                            tokenizer.decode(
                                op_without_suffix.sequences[0][
                                    len(tokenized_input.input_ids[0]) + ntk
                                ]
                            )
                        )
                        tks_with.append(
                            tokenizer.decode(
                                op_with_suffix.sequences[0][
                                    len(tokenized_attack_input.input_ids[0]) + ntk
                                ]
                            )
                        )

                        ante_pos = [
                            (i, tokens_with_suffix[i])
                            for i in suppressed_rule_token_pos
                        ]
                        ...
                        attns_without_suffix = [
                            attns_without_suffix_layer.max(dim=0).values
                            for attns_without_suffix_layer in attns_without_suffix
                        ]
                        attns_with_suffix = [
                            attns_with_suffix_layer.max(dim=0).values
                        ...
                        heatmap_without_suffix_padded = np.zeros(
                            heatmap_with_suffix.shape
                        )
                        heatmap_without_suffix_padded[
                            : heatmap_without_suffix.shape[0],
                            : heatmap_without_suffix.shape[1],
                        ] = heatmap_without_suffix

                        # Subtract the two heatmaps
                        heatmap_diff = (
                            heatmap_with_suffix - heatmap_without_suffix_padded
                        )
                        ...
                        print(
                            f"Attn without: {np.max(heatmap_without_suffix[:, ante_pos])}"
                        )
                        print(f"Attn with: {np.max(heatmap_with_suffix[:, ante_pos])}")

                        for l in range(12):
                            sample_attns_with_suffix[l] = max(
                                sample_attns_with_suffix[l],
                                np.max(heatmap_with_suffix[l, ante_pos]),
                            )
                            sample_attns_without_suffix[l] = max(
                                sample_attns_without_suffix[l],
                                np.max(heatmap_without_suffix[l, ante_pos]),
                            )
                            sample_attns_diff[l] = max(
                                sample_attns_diff[l], np.max(heatmap_diff[l, ante_pos])
                            )
                ...
                d = {
                    "Reasoner": [f"t={param_instance['num_steps']}", ""],
                    "Att. Wt.": ["Without suffix", "With suffix"],
                }
                for l in range(12):
                    ...
                    print(
                        f"All attns with suffix (mean: {sum(all_attns_with_suffix[l])/len(all_attns_with_suffix[l])}): {all_attns_with_suffix[l]}"
                    )
                    print(
                        f"All attns without suffix (mean: {sum(all_attns_without_suffix[l])/len(all_attns_without_suffix[l])}): {all_attns_without_suffix[l]}"
                    )
                    print(
                        f"All attns diff (mean: {sum(all_attns_diff[l])/len(all_attns_diff[l])}): {all_attns_diff[l]}"
                    )

                    print("\n---------------------\n")

                    d[f"{l}"] = [
                        sum(all_attns_without_suffix[l])
                        / len(all_attns_without_suffix[l]),
                        sum(all_attns_with_suffix[l]) / len(all_attns_with_suffix[l]),
                    ]
                df = pd.DataFrame(d)
