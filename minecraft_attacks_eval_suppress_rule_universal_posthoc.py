...
def generate_text_batch(model, tokenizer, prompts, max_length=1024, device="cuda", default_response="<PROMPT_TOO_LONG>", batch_size=1):
    input_ids = tokenizer(prompts, return_tensors="pt", padding="longest", truncation=True).to(device)
    with torch.no_grad():
    ...
    for param_dict in config:
        ...
        for param_tuple in itertools.product(*param_dict.values()):
            ...
            for sample in tqdm(samples, desc="Running attack on the eval dataset"):
                ...
                rule_supressed_in_sample = sorted(recipe_suppressed, key=lambda x: (x[2], x[1]))
                # Sort antecedents in each rule
                rule_supressed_in_sample = tuple((tuple(sorted(antecedents)), consequent, depth) for (antecedents, consequent, depth) in rule_supressed_in_sample)
                ...
                generations = generate_text_batch(model, tokenizer, prompts, device=args.device, batch_size=len(prompts))

                recipe_from_idx = sample["recipe_suppressed"]

                adv_tgts, recipes_suppressed, suppressed_strings = zip(*map(get_rule_suppressed_adv_target, generations, [recipe_from_idx] * len(generations), [True] * len(generations)))

                suppressed_strings_in_generation = map(generation_not_contains_label, suppressed_strings, generations)
                suppressed_strings_in_generation = [not s for s in suppressed_strings_in_generation]

                generations = [generation.split("I cannot create any other items.")[0] for generation in generations]

                attack_prompts = [f"{prompt}{sample['final_adversarial_suffix']}" for prompt in prompts]
                attack_generations = generate_text_batch(model, tokenizer, attack_prompts, device=args.device, batch_size=len(attack_prompts))

                
                attack_generation_matches_adv_target = list(map(generation_matches_label, adv_tgts, attack_generations))
                suppressions_successful = list(map(generation_not_contains_label, suppressed_strings, attack_generations))
                ...
                print(f"Train attack success percentage: {sample['attack_success_percentage']}")
                print(f"Number of suppressed strings in original generations: {num_suppressions_in_original}/{len(recipes)}")
                ...
                sample["percentage_suppressions_in_original"] = num_suppressions_in_original / len(recipes)
                ...
                sample["other_examples"] = [
                    ...
                    for ap, l, og, at, ss, ag, ssuc, agmat, ssig 
                    in zip(
                        attack_prompts, 
                        labels,
                        generations,
                ...
                percentage_successful_suppressions.append(num_successful_suppressions/len(recipes))
                percentage_successful_attacks.append(num_successful_attacks/len(recipes))

            # Save the samples with the results
            with open(os.path.join(args.output_dir, f"suppressed_results_batched_{exp_id}.json"), "w") as f:
                    json.dump(samples, f)

            print(f"Mean suppression percentage: {sum(percentage_successful_suppressions)/len(percentage_successful_suppressions)}")
            print(f"Max suppression percentage: {max(percentage_successful_suppressions)}")
            print(f"Min suppression percentage: {min(percentage_successful_suppressions)}")

            print(f"Mean attack success percentage: {sum(percentage_successful_attacks)/len(percentage_successful_attacks)}")
            print(f"Max attack success percentage: {max(percentage_successful_attacks)}")
            print(f"Min attack success percentage: {min(percentage_successful_attacks)}")